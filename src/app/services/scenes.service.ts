...
export class ScenesService {
      ...
      this.hideExpired$(
        this.jobStatusFilter$(
          this.hideS1Raw$(
            this.filterBaselineValues$(
              this.filterByDate$(
                this.filterBurstSubproducts$(this.store$.select(getScenes)),
              ),
            ),
          ),
        ),
  ...
  public unfilteredScenes$: Observable<CMRProduct[]> =
    this.store$.select(getScenes);

  public withBrowses$(
    scenes$: Observable<CMRProduct[]>,
  ): Observable<CMRProduct[]> {
    return scenes$.pipe(
  ...
  public sortScenes$(scenes$: Observable<CMRProduct[]>) {
    return combineLatest([
      ...
      map(([scenes, tempSort, perpSort]) => {
        if (
          tempSort === ColumnSortDirection.NONE &&
          perpSort === ColumnSortDirection.NONE
        ) {
          return scenes;
        }
    ...
    return combineLatest([
      ...
      map(([scenes, showS1RawData, productTypes, searchType]) => {
        ...
        const filteredScenes = scenes.filter(
          (scene) => !scene.productTypeDisplay.includes('RAW'),
        );
  ...
  private filterBurstSubproducts$(scenes$: Observable<CMRProduct[]>) {
    return combineLatest([scenes$, this.store$.select(getSearchType)]).pipe(
      map(([scenes, searchtype]) => {
        if (searchtype === SearchType.CUSTOM_PRODUCTS) {
          return scenes.filter(
            (scene) => scene.productTypeDisplay !== 'XML Metadata (BURST)',
          );
        }
        return scenes;
      }),
  ...
  private filterByDate$(scenes$: Observable<CMRProduct[]>) {
    return combineLatest([
      ...
      map(([scenes, dateRange, searchType]) => {
        if (searchType !== SearchType.CUSTOM_PRODUCTS) {
          return scenes;
        }
  ...
  private dateInRange(date: moment.Moment, range: Range<Date | null>): boolean {
    return this.after(date, range.start) && this.before(date, range.end);
  }
  ...
  private sceneHasBrowse(scene: CMRProduct): boolean {
    return (
      scene.browses.filter((browse) => !browse.includes('no-browse')).length > 0
    );
  }

  private after(check: moment.Moment, pivot: Date | null): boolean {
    return pivot === null || check.isAfter(moment.utc(pivot));
  }

  private before(check: moment.Moment, pivot: Date | null): boolean {
    return pivot === null || check.isBefore(moment.utc(pivot));
  }

  public isExpired(job: Hyp3Job): boolean {
    return (
      job.status_code === Hyp3JobStatusCode.SUCCEEDED &&
    ...
    return combineLatest([
      ...
      map(([scenes, searchType, productNameFilter]) => {
        if (searchType === SearchType.CUSTOM_PRODUCTS && !!productNameFilter) {
          ...
          fileIds = fileIds
            .map((id) => id.toLowerCase().trim().split('.')[0])
          ...
          return scenes.filter((scene) => {
            ...
            return (
              fileIds.some(
                (id) => fileName.includes(id) || sourceGranule.includes(id),
              ) ||
