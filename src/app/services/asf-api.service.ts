...
export class AsfApiService {
  ...
  public query<T>(
    stateParamsObj: Record<string, string | number | null>,
  ): Observable<T> {
    ...
    const queryParamsStr = params.toString().replace(/\+/g, '%2B');
    ...
    const responseType: any =
      params.get('output') === 'jsonlite2' ? 'json' : 'text';
  ...
  private queryParamsFrom(stateParamsObj) {
    ...
    const params = Object.entries(this.baseParams())
      .filter(([key, _]) => !stateParams.get(key))
      .reduce(
  ...
  private toFormData(params: HttpParams): FormData {
    ...
    params.keys().forEach((key) => formData.append(key, params.get(key)));
  ...
  private onlyRelevantParams(paramsObj): Record<string, string | null> {
    const irrelevant = Object.entries(apiParamNames)
      .filter(([property, _]) => !this.prop.isRelevant(property as Props))
    ...
    const filteredParams = Object.keys(paramsObj)
      ...
      .reduce((filtered, key) => {
        if (
          key === 'processinglevel' &&
          this.prop.isRelevant(Props.USE_PROCESSING_TYPE)
        ) {
  ...
  public missionSearch(
    ...
    const params = new HttpParams().append('platform', dataset);
  ...
  public upload(files): Observable<any> {
    ...
    return this.http.post(
      `${this.apiUrl}/services/utils/files_to_wkt`,
      formData,
    );
  }

  public validate(wkt: string): Observable<any> {
    const params = new HttpParams().append('wkt', wkt);
