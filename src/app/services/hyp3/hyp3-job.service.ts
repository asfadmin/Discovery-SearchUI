...
export class Hyp3JobService {
  public getAllGranulesFromJobs(jobs: models.Hyp3Job[]): string[] {
    return jobs.reduce((granuleNames, job) => {
  ...
  public formatJobs(
    ...
    models.hyp3JobTypesList.forEach(
      (jobType) =>
        (jobOptionNames[jobType.id] = new Set(
    ...
    models.hyp3JobTypesList.forEach((jobType) => {
      ...
      Object.entries(options.processingOptions[jobType.id]).forEach(
        ([name, value]) => {
    ...
    return jobs.map((job) => {
      ...
      if (job.job_type.id === 'ARIA_S1_GUNW') {
        ...
        if (job.granules.length > 0) {
          const g1 = moment.isMoment(job.granules[0].metadata.date)
            ? job.granules[0].metadata.date.format('YYYY-MM-DD')
            : moment(job.granules[0].metadata.date).format('YYYY-MM-DD');
          const g2 = moment.isMoment(job.granules[1].metadata.date)
            ? job.granules[1].metadata.date.format('YYYY-MM-DD')
            : moment(job.granules[1].metadata.date).format('YYYY-MM-DD');
          const swap = g1 > g2;
  ...
  public toDummyCMRProducts(jobs: models.Hyp3Job[]) {
    ...
    const dummyProducts = this.dummyProductsFor(granuleNames).reduce(
      (prodsByName, p) => {
    ...
    const virtualProducts = jobs.map((job) => {
      ...
      if (jobGranules.length < 1) {
        ...
        if (
          ...
          product['metadata']['date'] = moment(
            job.job_parameters['reference_date'],
          );

          const product2 = this.dummyProduct();
          product2['isDummyProduct'] = false;
          product2['metadata']['date'] = moment(
            job.job_parameters['secondeary_date'],
          );
        }
      } else {
      ...
      job.scenes = jobGranules.map((granuleName) => dummyProducts[granuleName]);
  ...
  public combineWithCmrProduct(
    oldJobProducts: models.CMRProductsById,
    cmrData: models.CMRProductsById,
  ): models.CMRProductsById {
    ...
    Object.values(newJobProducts).forEach((jobProduct) => {
      ...
      job.scenes = jobGranules.map((granuleName) => cmrData[granuleName]);
  ...
  public combineJobAndCmrProduct(
    ...
    const combinedProduct: any = {
      ...
      thumbnail: job.thumbnail_images
        ? job.thumbnail_images[0]
        : 'assets/no-thumb.png',
