...
export class PossibleHyp3JobsService {
  ...
  private possibleProductJobs$ = this.products$.pipe(
    map((products) => products.map((p) => [p])),
  );
  ...
  private hyp3ableJobsSBAS$ = combineLatest([
    this.pairService.productsFromPairs$.pipe(
      map((products) => products.map((p) => [p])),
    ),
    this.pairs$,
  ]).pipe(
  ...
  private makeBaselinePairs(
    products: models.CMRProduct[],
    referenceScene: models.CMRProduct,
  ) {
