  ...
  public pollHyp3Jobs$(
    ...
    const inProgressScenes$ = scenes$.pipe(
      map((scenes) =>
        scenes.filter(
          (scene) =>
            scene.metadata?.job?.status_code ===
              models.Hyp3JobStatusCode.RUNNING ||
            scene.metadata?.job?.status_code ===
              models.Hyp3JobStatusCode.PENDING,
        ),
      ),
    );
    ...
    const pollRunningJobs$ = combineLatest([
      ...
      switchMap(([_, inProgressScenes, hyp3UserId]) => {
        return forkJoin([
          this.hyp3.getJobs$({
            userID: hyp3UserId,
            statusCode: models.Hyp3JobStatusCode.PENDING,
          }),
          this.hyp3.getJobs$({
            userID: hyp3UserId,
            statusCode: models.Hyp3JobStatusCode.RUNNING,
          }),
          of({ inProgressScenes, hyp3UserId }),
        ]).pipe(
          repeat({ delay: this.pollingRepeatTime }),
          filter(
            ([__, ___, oldRunningJobs]) =>
              oldRunningJobs.hyp3UserId === hyp3UserId,
          ),
          map(([pending, running, oldRunningJobs]) => {
            return (
              oldRunningJobs.inProgressScenes.length -
              (running.hyp3Jobs.length + pending.hyp3Jobs.length)
            );
          }),
        );
      }),
