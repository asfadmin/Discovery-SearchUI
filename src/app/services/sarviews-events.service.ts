...
import {
  getSarviewsEvents,
  getSelectedSarviewsEventProducts,
} from '@store/scenes';
...
export class SarviewsEventsService {
  ...
  public getSarviewsEvents$ = this.http
    .get<models.SarviewsEvent[]>(this.eventsUrl)
    .pipe(
      debounceTime(2000),
      distinctUntilChanged(),
      map((events) =>
        events.filter((event) => event.event_type.toLowerCase() !== 'flood'),
      ),
      map((events) =>
        events.map((event) => {
          return {
            ...event,
            processing_timeframe: event.processing_timeframe
              ? this.getDates(event)
              : null,
      ...
      map((events) =>
        events.sort((a, b) => {
          if (a.processing_timeframe.start > b.processing_timeframe.start) {
            ...
            a.processing_timeframe.start < b.processing_timeframe.start
          ) {
            return 1;
          }
          return 0;
  ...
  public getEventFeature(
    usgs_id: string,
  ): Observable<models.SarviewsProcessedEvent> {
    return this.http
      .get<models.SarviewsProcessedEvent>(this.eventsUrl + '/' + usgs_id)
      .pipe(
  ...
  public quakeIds$() {
    return this.getSarviewsEvents$.pipe(
      map((events) =>
        events.filter((sarviewsEvent) => sarviewsEvent.event_type === 'quake'),
      ),
  ...
  public volcanoIds$() {
    return this.getSarviewsEvents$.pipe(
      map((events) =>
        events.filter(
          (sarviewsEvent) => sarviewsEvent.event_type === 'volcano',
        ),
      ),
  ...
  public floodIds$() {
    return this.getSarviewsEvents$.pipe(
      map((events) =>
        events.filter((sarviewsEvent) => sarviewsEvent.event_type === 'flood'),
      ),
      map((volcanoEvents) => volcanoEvents.map((volcano) => volcano.event_id)),
    );
  }
  ...
  private getDates(
    event: models.SarviewsEvent | models.SarviewsProcessedEvent,
  ): Range<Date> {
  ...
  public getSourceCMRProducts(
    product: models.SarviewsProduct,
  ): models.CMRProduct[] {
  ...
  public eventProductToCMRProduct(
    product: models.SarviewsProduct,
  ): models.CMRProduct {
    return this.toCMRProduct(product, product.granules[0]);
  }

  private toCMRProduct(
    product: models.SarviewsProduct,
    granule: models.SarviewProductGranule,
  ) {
  ...
  public hyp3able(
    product: models.SarviewsProduct,
  ): models.Hyp3ableProductByJobType {
  ...
  public toHyp3ableProducts(products: models.SarviewsProduct[]): {
    ...
    Object.keys(hyp3ProductsByType).forEach((key) =>
      byProductType.push({
        productType: key,
        products: hyp3ProductsByType[key],
      }),
    );

    const output: models.Hyp3ableProductByJobType[] = Object.keys(
      hyp3ProductsByType,
    ).map((key) => ({
      jobType: models.hyp3JobTypes[key],
      byProductType: byProductType.filter((prod) => prod.productType === key),
      total: byProductType.find((prod) => prod.productType === key).products
        .length,
    }));

    return {
      byJobType: output,
      total: output.reduce((prev, curr) => (prev += curr.total), 0),
    };
  }

  public filteredSarviewsEvents$(): Observable<models.SarviewsEvent[]> {
    return this.filterByEventMagnitude$(
      this.filterSarviewsEventsByName$(
        this.filterByEventType$(
          this.filterByEventDate$(
            this.filterByEventActivity$(this.store$.select(getSarviewsEvents)),
          ),
        ),
      ),
    );
  ...
  private filterByProductDate$(
    products$: Observable<models.SarviewsProduct[]>,
  ) {
    return combineLatest([
      products$,
      this.store$.select(filtersStore.getDateRange),
    ]).pipe(
      map(([products, dateRange]) =>
  ...
  private filterByProductPathFrame$(
    products$: Observable<models.SarviewsProduct[]>,
  ) {
    return combineLatest([
      products$,
  ...
  private filterByProductType$(
    ...
    return combineLatest([
      ...
      this.store$
        ...
        .pipe(map((jobTypes) => jobTypes.map((jobType) => jobType.id))),
    ]).pipe(
      map(([products, jobTypes]) => {
        ...
        return products.filter((product) =>
          jobTypes.includes(product.job_type),
        );
      }),
    );
  }
  ...
  private ProductSortOrder$(products$: Observable<models.SarviewsProduct[]>) {
    return combineLatest([
      ...
      map(([products, sorting]) => {
        ...
        if (
          sorting.sortDirection === models.EventProductSortDirection.ASCENDING
        ) {
  ...
  public filterSarviewsEventsByName$(
    events$: Observable<models.SarviewsEvent[]>,
  ) {
    return combineLatest([
      events$,
      this.store$
        .select(filtersStore.getSarviewsEventNameFilter)
        .pipe(map((nameFilter) => nameFilter?.toLowerCase())),
    ]).pipe(
      map(([events, nameFilter]) => {
        ...
        return events.filter((event) => {
          ...
          return (
            (event.description?.toLowerCase().includes(nameFilter) ?? false) ||
            event.event_id.toLowerCase().includes(nameFilter) ||
            event.event_type.toLowerCase().includes(nameFilter) ||
            (!isVolcano
              ? isQuake
                ? (event as models.SarviewsQuakeEvent).usgs_event_id?.includes(
                    nameFilter,
                  )
                : true
              : (
                  event as models.SarviewsVolcanicEvent
                ).smithsonian_event_id?.includes(nameFilter))
          );
        });
      }),
    );
    ...
    return combineLatest([
      ...
      map(([events, dateRange]) => {
        ...
        return events.filter((scene) => {
          ...
          if (dateRange.start === null && dateRange.end !== null) {
            return (
              processing_start <= moment(dateRange.end) ||
              processing_end <= moment(dateRange.end)
            );
          } else if (dateRange.start !== null && dateRange.end === null) {
            return (
              processing_start >= moment(dateRange.start) ||
              processing_end >= moment(dateRange.start)
            );
          } else {
            const processingRange = this.Moment.range(
              processing_start,
              processing_end,
            );
    ...
    return combineLatest([
      ...
      map(([events, types]) => {
        ...
        return events.filter(
          (event) => !!types.find((t) => t === event.event_type.toLowerCase()),
        );
      }),
    );
  }
