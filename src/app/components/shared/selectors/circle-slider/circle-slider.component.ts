...
export class CircleSliderComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.startValue && changes.endValue) {
      if (
        changes.startValue.previousValue !== undefined &&
        changes.endValue.previousValue !== undefined
      ) {
        ...
        this.startDot.attr(
          'cx',
          this.circumference_r * Math.sin(this.startAngle),
        );
        this.startDot.attr(
          'cy',
          -this.circumference_r * Math.cos(this.startAngle),
        );
      ...
      if (
        changes.startValue &&
        changes.startValue.currentValue !== this.getPoint(this.startAngle)
      ) {
        this.startAngle = this.getAngle(changes.startValue.currentValue);
        this.startDot.attr(
          'cx',
      ...
      if (
        changes.endValue &&
        changes.endValue.currentValue !== this.getPoint(this.endAngle)
      ) {
        this.endAngle = this.getAngle(changes.endValue.currentValue);
        this.endDot.attr('cx', this.circumference_r * Math.sin(this.endAngle));
        this.endDot.attr('cy', -this.circumference_r * Math.cos(this.endAngle));
  ...
  private setAngles() {
    ...
    for (const line of this.lines) {
      ...
      } else if (startAngle < line.angle && line.angle < endAngle) {
        line.line.attr('class', 'notch selected');
      } else {
        line.line.attr('class', 'notch');
      }
  ...
  private movePoint(target, event: { x: number; y: number }, _d?: any) {
    const d_from_origin = Math.sqrt(
      Math.pow(event.x, 2) + Math.pow(event.y, 2),
    );
    ...
    d3.select(target)
      ...
      .attr(
        'cy',
        event.y < 0
          ? -this.circumference_r * Math.sin(alpha)
          : this.circumference_r * Math.sin(alpha),
      );
  }
  ...
  private drawSlider() {
    ...
    for (let i = 0; i < sections; i++) {
      ...
      const line_container = container
        .append('path')
        .attr('d', line)
        .attr('class', 'notch');
    ...
    this.arc = d3
      .arc()
      .outerRadius(this.circumference_r)
      .innerRadius(this.circumference_r)
      .startAngle(self.startAngle)
      .endAngle(self.endAngle);

    this.arcContainer = container
      .append('path')
      .attr('d', self.arc)
      .attr('class', 'range ');
    this.setAngles();
