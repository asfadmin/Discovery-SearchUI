...
export class HelpTocComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.YoutubeApiService$.getVideosForChannel().subscribe(
      (videoList) => {
  ...
  public matchContentDetails() {
    for (const video of this.videos) {
      ...
      this.YoutubeApiService$.getVideoContentDetails(
        video.id.videoId,
      ).subscribe(
        (detailList) => {
          for (const detail of detailList['items']) {
            for (const vid of this.videos) {
              if (vid.id.videoId === detail.id) {
                detail.contentDetails.duration = this.convertISO8601ToMs(
  ...
  public matchTags() {
    for (const video of this.videos) {
      this.YoutubeApiService$.getTagsForVideo(video.id.videoId).subscribe(
        (tagList) => {
  ...
  public loadDataTables() {
    ...
    this.searchDataSource.filterPredicate = (data: any, filter: string) =>
      this.tagFilter(data, filter);
    ...
    this.resultsDataSource.filterPredicate = (data: any, filter: string) =>
      this.tagFilter(data, filter);
    ...
    this.featuresDataSource.filterPredicate = (data: any, filter: string) =>
      this.tagFilter(data, filter);
