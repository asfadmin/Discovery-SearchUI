...
export class BaselineChartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subs.add(
      combineLatest([
      ...
      ]).subscribe(([points, selected]) => {
        const extrema = this.determineMinMax(points);
        const { minDataset, maxDataset } =
          this.criticalBaselineDataset(extrema);
        ...
        if (this.isFirstLoad) {
          ...
          const height =
            this.y(this.data[ChartDatasets.MIN_CRITICAL][0].y) -
            this.y(this.data[ChartDatasets.MAX_CRITICAL][1].y);
          ...
          this.criticalBoxContainer
            ...
            .attr(
              'width',
              this.x(this.data[ChartDatasets.MAX_CRITICAL][1].x) -
                this.x(this.data[ChartDatasets.MIN_CRITICAL][0].x),
            )
    ...
    this.subs.add(
      combineLatest([
        ...
        .pipe(
          map(([masterName, products]) =>
            products.filter((product) => product.name === masterName).pop(),
          ),
  ...
  private createSVG() {
    ...
    this.height =
      this.baselineChart.nativeElement.offsetHeight -
      this.margin.top -
      this.margin.bottom;
    this.width =
      this.baselineChart.nativeElement.offsetWidth -
      this.margin.left -
      this.margin.right;
    this.svg = d3
      .select(this.baselineChart.nativeElement)
      .append('svg')
  ...
  private drawChart() {
    ...
    this.criticalBoxContainer = this.clipContainer
      .append('g')
      .append('rect')
  ...
  private updateChart() {
    ...
    this.dotsContainer
      ...
      .attr('cy', (d) => newY(d.y));
    const height =
      newY(this.data[ChartDatasets.MIN_CRITICAL][0].y) -
      newY(this.data[ChartDatasets.MAX_CRITICAL][1].y);
    this.criticalBoxContainer
      .attr('x', newX(this.data[ChartDatasets.MIN_CRITICAL][0].x))
      .attr('y', newY(this.data[ChartDatasets.MAX_CRITICAL][1].y))
      .attr(
        'width',
        newX(this.data[ChartDatasets.MAX_CRITICAL][1].x) -
          newX(this.data[ChartDatasets.MIN_CRITICAL][0].x),
      )
      .attr('height', isNaN(height) ? 0 : height);
  ...
  private updateCircles() {
    ...
    this.dotsContainer
      ...
      .attr('class', function (d) {
        if (
          self.data[ChartDatasets.MASTER].length > 0 &&
          self.data[ChartDatasets.MASTER][0]?.id === d.id
        ) {
          return 'baseline-reference';
        } else if (
          self.data[ChartDatasets.SELECTED].length > 0 &&
          self.data[ChartDatasets.SELECTED][0]?.id === d.id
        ) {
          return 'baseline-selected';
        } else if (
          self.data[ChartDatasets.DOWNLOADS].some((p) => p.id === d.id)
        ) {
  ...
  private updateTooltip() {
    ...
    this.tooltip
      .style('left', `${bounding.x + (a ? -120 : 20)}px`)
      .style('top', `${bounding.y - 10}px`);
  }
  private setDataset(dataset: ChartDatasets, data) {
    this.data[dataset] = data;
    if (
      dataset === ChartDatasets.PRODUCTS ||
      dataset === ChartDatasets.DOWNLOADS
    ) {
      this.updateCircles();
    }
  }
  ...
  private updateScales(extrema: {
    ...
    const { min, max } = extrema;
    const xBuffer = Math.floor((max.x - min.x) * 0.25);
    const yBuffer = Math.floor((max.y - min.y) * 0.25);
    this.xExtent = [min.x - xBuffer, max.x + xBuffer];
    this.x = d3.scaleLinear().domain(this.xExtent).range([0, this.width]);
