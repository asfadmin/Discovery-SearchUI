...
export class SearchEffects {
  ...
  public clearMapInteractionModeOnSearch = createEffect(() =>
    this.actions$.pipe(
  ...
  public closeMenusWhenSearchIsMade = createEffect(() =>
    this.actions$.pipe(
      ofType(SearchActionType.MAKE_SEARCH),
      switchMap((_) => [
  ...
  public resetMoreJobsToLoadOnSearch = createEffect(() =>
    this.actions$.pipe(
      ofType(SearchActionType.MAKE_SEARCH),
  ...
  public setCanSearch = createEffect(() =>
    this.actions$.pipe(
  ...
  public onUpdateMaturity = createEffect(
    () =>
      this.actions$.pipe(
  ...
  public getNextJobBatch = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([action, [scenes, maxHyp3Jobs]]) => {
  ...
  public cancelSearchWhenFiltersCleared = createEffect(() =>
    this.actions$.pipe(
      ofType(
  ...
  public cancelSearchonOnPanelOpen = createEffect(() =>
    this.actions$.pipe(
  ...
  public searchResponse = createEffect(() =>
    this.actions$.pipe(
  ...
  public onDemandSearchResponse = createEffect(() =>
    this.actions$.pipe(
      ofType<SearchResponse>(SearchActionType.SEARCH_RESPONSE),
      withLatestFrom(this.store$.select(getSearchType)),
  ...
  public onLoadOnDemandScenesList = createEffect(() =>
    this.actions$.pipe(
      ofType<LoadOnDemandScenesList>(
        SearchActionType.LOAD_ON_DEMAND_SCENES_LIST,
      ),
      ...
      switchMap((action) => {
        ...
        const granuleNames = products.reduce((names, prod) => {
          ...
          if (scenes) {
            ...
            return names.concat(
              prod.metadata.job.files[0]?.filename.slice(0, -3),
            );
          } else {
      ...
      map(([asfApiResp, products]) => {
        const results = this.productService
          .fromResponse(asfApiResp)
          .filter(
            (product) => !product.metadata.productType?.includes('METADATA'),
          );
        ...
        const combinedProducts = this.hyp3JobService.combineWithCmrProduct(
          products,
          cmrData,
        );
  ...
  public hyp3BatchResponse = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) => [
        ...
        action.payload.next
          ? new SetNextJobsUrl(action.payload.next)
          : new SetNextJobsUrl(''),
      ]),
    ),
  );
  ...
  public timeseriesSearchResponse = createEffect(() =>
    this.actions$.pipe(
      ofType<TimeseriesSearchResponse>(
        SearchActionType.DISPLACEMENT_SEARCH_RESPONSE,
      ),
      withLatestFrom(this.store$.select(getSearchType)),
  ...
  public showTimeseriesResultsMenuOnSearchResponse = createEffect(() =>
    this.actions$.pipe(
      ofType<TimeseriesSearchResponse>(
        SearchActionType.DISPLACEMENT_SEARCH_RESPONSE,
      ),
    ...
    this.actions$.pipe(
      ...
      switchMap((action) => {
        ...
        if (action.payload === models.SearchType.DISPLACEMENT) {
          output.push(
            new mapStore.SetMapDrawMode(models.MapDrawModeType.POINT),
          );
        } else {
  ...
  public clearResultsWhenSearchTypeChanges = createEffect(() =>
    this.actions$.pipe(
      ...
      catchError((_) => of(new SearchError('Error loading search results'))),
    ),
  );

  public onChangeFiltersHeader = createEffect(() =>
    this.actions$
      .pipe(
        ofType(
        ...
        filter(
          ([[[_, searchtype], outOfdate], loaded]) =>
            !outOfdate && searchtype === models.SearchType.DATASET && loaded,
        ),
      )
      .pipe(map((_) => new SetSearchOutOfDate(true))),
  );

  public setSearchUpToDate = createEffect(() =>
    this.actions$.pipe(
      ofType(
  ...
  public onSetSearchOutOfDate = createEffect(
    () =>
      this.actions$.pipe(
        ...
        tap((_) =>
          this.notificationService.info(
            'Refresh search to show new results',
            'Results Out of Date',
          ),
        ),
      ),
  ...
  public OnSetKioskMode = createEffect(() =>
    this.actions$.pipe(
      ofType<setSearchKioskMode>(SearchActionType.SET_SEARCH_KIOSK_MODE),
      filter((action) => action.payload),
      switchMap((_) => [
        new SetSearchType(SearchType.DISPLACEMENT),
  ...
  private asfApiQuery$ = this.searchParams.getParams.pipe(
    ...
    map((params) => [params]),
    switchMap(([params]) =>
      forkJoin(this.asfApiService.query<any[]>(params)).pipe(
        withLatestFrom(
          combineLatest([
  ...
  public asfApiBaselineQuery$(): Observable<Action> {
    return this.searchParams.getParams.pipe(
      switchMap((params) => {
        const apiQuery$ = this.asfApiService
          .query<any[]>(params)
          .pipe(map((response) => this.productService.fromResponse(response)));
    ...
    return this.searchParams.onDemandParams$.pipe(
      switchMap((params) => {
        ...
        if (params.jobIds?.length > 0) {
          ...
          hyp3Query = this.hyp3Service.getJobs$({
        ...
        return hyp3Query.pipe(
          switchMap((jobsRes: { hyp3Jobs: models.Hyp3Job[]; next: string }) => {
            if (jobsRes.hyp3Jobs.length === 0 && jobsRes.next === '') {
              return of(
                new SearchResponse({
                  files: [],
        ...
        if (this.vectorSource.getFeatures().length > 0) {
          ...
          this.http.get('/assets/countries.geojson').subscribe((f) => {
            this.vectorSource.addFeatures(
              this.vectorSource
                .getFormat()
                .readFeatures(f) as Feature<Geometry>[],
            );
            this.findCountries(params.intersectsWith);
          });
        }
