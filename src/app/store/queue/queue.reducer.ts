...
export function queueReducer(
  ...
  switch (action.type) {
    case QueueActionType.ADD_ITEMS: {
      ...
      newProducts.forEach((product) => (products[product.id] = product));
    ...
    case QueueActionType.REMOVE_ITEMS: {
      ...
      const ids = [...state.ids].filter((id) => !toRemove.has(id));
      let downloads = { ...state.downloads };
      toRemove.forEach((id) => {
    ...
    case QueueActionType.CLEAR_PROCESSING_QUEUE_BY_JOB_TYPE: {
      ...
      const customJobs = jobs.filter((job) => !jobTypes.has(job.job_type.id));
      return {
        ...state,
        customJobs,
      };
    ...
    case QueueActionType.ADD_JOBS: {
      ...
      let _duplicates = 0;
      const jobsToQueue = new_jobs.filter(
        (new_job) =>
          !jobs.some((old_job) => {
            const result =
    ...
    case QueueActionType.REMOVE_JOB: {
      const queue = [...state.customJobs].filter(
        (job) =>
          !(
    ...
    case QueueActionType.REMOVE_JOBS: {
      let queue = [...state.customJobs].filter(
        (queueJob) => 'job_type' in queueJob,
      );
...
const remove_product = (toRemove, prods) =>
  Object.keys(prods)
...
export const getQueuedProducts = createSelector(
  getQueueState,
  (state: QueueState) =>
    state.ids.reduce((total, id) => [...total, state.products[id]], []),
);
