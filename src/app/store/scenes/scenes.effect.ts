...
export class ScenesEffects {
  ...
  public setSelectedSceneOnLoad = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([action, selected]) => {
        const products = action.products.reduce((total, product) => {
        ...
        if (action.searchType === SearchType.BASELINE) {
          Object.values(products).forEach((product: CMRProduct) => {
            if (
              product.metadata.temporal === 0 &&
              product.metadata.perpendicular === 0
            ) {
              current_selected = product.id;
            }
          });
        }
        const productGroups: Record<string, string[]> = action.products.reduce(
          (total, product) => {
  ...
  public onHideRawData = createEffect(
    () =>
      this.actions$.pipe(
