...
test('Anti-Meridian Granules', async ({ page }) => {
  ...
  expect(requestUrl).toContain('end=2010-07-04');
  expect(requestUrl).toContain('dataset=ALOS');

  const firstResult = page
    .getByRole('button', { name: /ALAV2A.*2010/ })
    .first();
  await expect(firstResult).toBeVisible({ timeout: 30000 });
  await expect(page).toHaveScreenshot('meridian-3-search-results.png');

  await firstResult.click();
  await expect(page.locator('mat-card-title')).toContainText('ALAV2A');
  await expect(page).toHaveScreenshot('meridian-3-result-selected.png');
});
